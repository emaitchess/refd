...
export const sliceAtAngle = (slices: PieSlice[], angle: number): number => {
...
export const radarAxes = (data: Row[], nameKey: string): RadarAxis[] => {
...
export const axisAtAngle = (axes: RadarAxis[], angle: number): number => {
  let best = 0
  let bestD = Infinity
  axes.forEach((ax, i) => {
...
export const distToPolygonEdge = (
  px: number,
  py: number,
  poly: number[]
): number => {
