    ...
    const paint = (prog: number) => {
      ...
      keys.forEach((key, si) => {
        ...
        for (let i = 0; i < s.dataLength; i++) {
          ...
          // Sparse keys in a stacked chart yield zero-height segments; skip
          // them entirely — paintColumn's zero-depth branch would still cap
          // them with a border pixel, leaking a baseline hairline in this
          // series' colour. Target geometry, not animated, so real bars still
          // grow from frame 0.
          if (stacked && base - (t.top[i] ?? base) < 0.5) continue
