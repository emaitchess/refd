...
export type ChartContextValue = {
  ...
  common: CommonChart // shared surface for <Legend> / <ChartTooltip>
}
...
export const useRevision = (data: unknown, token: number) => {
...
export const useChartController = ({
  ...
  // Memoized: this is the value handed to CommonChartContext (Legend/ChartTooltip),
  ...
  const common: CommonChart = useMemo(() => ({
    ...
    // Follow the highest hovered node, but reserve headroom for every tooltip
    // row. A fixed floor clipped the first series in wider comparisons.
    tooltipTop: (() => {
      const floor = mTop + Math.max(44, configKeys.length * 18 + 12)
