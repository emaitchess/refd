...
The production MCP endpoint is https://api.refd.ai/mcp. It uses OAuth with PKCE, and a grant covers the workspaces selected at consent: a checked set, or every workspace via Allow all (including ones created later). Access is read-only by default: nine analytics tools plus a metric-glossary resource. The optional data:write scope adds twelve setup tools plus revoke_connection that cover the whole lifecycle: create_workspace provisions a workspace (Allow all connections only, since checked grants can never target workspaces created after approval), check_domain verifies a candidate domain before it is saved, the get_setup_state/confirm_setup/get_setup_report group configures a workspace and starts its one provider-backed onboarding report, complete_setup finishes onboarding, and revoke_connection kills the grant itself and every token issued under it after an explicit confirm argument - the only destructive action a connector can perform, and only ever on its own connection. No grant can delete data, manage billing, or start further runs. Personal access tokens for headless agents stay read-only and single-workspace.
